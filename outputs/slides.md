...
- **Frozen in Time:** LLMs have training cut-offs (Llama 3.2 does not know fresh discoveries by default).
...
# Workshop Flow (Current Notebook)

Notebook sections now run in this order:

1. **Setup Ollama in Colab**
2. **Download source data** (Google Drive file)
3. **Intro to RAG** (no-RAG vs RAG)
...
# 💻 Workshop Time!
### Open: `technical_intro_to_rag.ipynb`
...
**The Solution:**
1. Grab the top 12 most "similar" chunks.
2. Use a **Reranker** (a "Cross-Encoder" model) to act as a judge.
3. Re-score those chunks and keep only the strongest few (current demo uses `top_n = 3`).
...
- **Next Steps:** Try changing chunk size, overlap, top-k, and reranker top-n to see how retrieval quality shifts.
